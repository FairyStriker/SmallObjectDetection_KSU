...
def main():
    ...
    # [수정 1] 안정적인 출발을 위해 학습률(LR) 하향 조정
    optimizer = optim.AdamW(student.parameters(), lr=0.0001, weight_decay=0.04)
    scaler = torch.amp.GradScaler('cuda')

    epochs = 15 
    print("🔥 16만 장 듀얼 GPU 집중 학습을 시작합니다... (Loss 고정 버그 수정 완료)")
    ...
    for epoch in range(epochs):
        for it, images in enumerate(data_loader): 
            ...
            # 빠른 확인을 위해 10 스텝마다 로그를 출력하도록 변경했습니다.
            if (it + 1) % 10 == 0: 
